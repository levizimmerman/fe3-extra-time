...
Every type attribute has a prefix: 'HKQuantityTypeIdentifier' or 'HKCategoryTypeIdentifier'. Hence the `OR` operator within the [`.replace()`][jsReplace] function. Also this function pushes the type to a global `types` array. In this way you could log which types of records are transformed.
```javascript
